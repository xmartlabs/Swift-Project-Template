...
extension NSURL {
    var fileName: String {
        ...
        try! getResourceValue(&fileName, forKey: kCFURLNameKey as URLResourceKey)
...
func checkThatProjectForlderCanBeCreated(projectURL: NSURL){
    ...
    if fileManager.fileExists(atPath: projectURL.path!, isDirectory: &isDirectory){
        printErrorAndExit(message: "\(projectName) \(isDirectory.boolValue ? "folder already" : "file") exists in \(runScriptPathURL.path ?? "(Empty path)") directory, please delete it and try again")
    }
}
...
let enumerator = fileManager.enumerator(at: newProjectFolderURL as URL, includingPropertiesForKeys: [URLResourceKey.nameKey, URLResourceKey.isDirectoryKey], options: [], errorHandler: nil)!
